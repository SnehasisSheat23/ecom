...
const EXACT_PRODUCT_R2_MAP: Record<string, string> = {
  ...
  'tomato-ketchup-squeeze': `${R2_DOMAIN}/tomato_ketchup_squeeze_pack_1786346843906.png`,
  'sweet-corn-canned': `${R2_DOMAIN}/sweet_corn_canned_pack_1786346860827.png`,
  'cheddar-cheese-sauce': `${R2_DOMAIN}/cheddar_cheese_sauce_pack_1786346876424.png`,
...
async function updateAllProductsWithExactGeneratedImages() {
  ...
  for (const p of prods) {
    ...
    if (exactUrl) {
      ...
      // 2. Clear old product images for this product
      await db.delete(productImages).where(eq(productImages.productId, p.id))

      // 3. Insert fresh media asset and link
      const [media] = await db.insert(mediaAssets).values({
        tenantId: abTenantId,
        url: exactUrl,
        filename: `${p.slug}-exact-generated-pack.png`,
        mimeType: 'image/png',
        sizeBytes: 0,
      }).returning()
      ...
      console.log(`Updated exact product photo for ${p.title} (${p.slug}) -> ${exactUrl}`)
    }
  }
}

updateAllProductsWithExactGeneratedImages().catch(console.error)
