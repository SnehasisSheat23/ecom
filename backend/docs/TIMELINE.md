...
**Async module rule:** Any module that introduces delayed jobs or background delivery is not done until retry policy, idempotency behavior, publish-failure logging, and one end-to-end integration test are all in place.
...
- App-level Filtering: All repositories must accept and filter by `tenant_id`
...
- Tenant-prefixed async queue name helpers (`src/lib/queue-names.ts`)
...
**Common mistake:** Don't skip the helper files. If queue names, cache keys, and storage paths aren't prefixed from day one, you will have a painful migration at tenant #15.
...
- Reservation: decrement `available`, increment `reserved`, schedule delayed release job via queue adapter
...
- Cart expiry delayed job (7-day inactivity)
...
- `NotificationService.send()` — always enqueues async job, never sends inline
- Notification delivery endpoint/worker handler
...
- Delayed abandonment job scheduled on cart create/update
...
- Earn: async job enqueued when order → DELIVERED
...
- Sync async job: product create/update/delete → Typesense index
